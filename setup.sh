...
bootstrap_agent() {
  ...
  if [[ "$name" == "claude" ]]; then
    log "Writing CLAUDE.md stub (imports $DOCS_PATH/agents/AGENTS.md)..."
    rm -f "$agent_dir/CLAUDE.md"
    cat > "$agent_dir/CLAUDE.md" << EOF
# Memex — Claude Code Entry Point

This stub is generated by setup.sh. The real system definition, memory, and skills live in the vault at $DOCS_PATH/agents/ so they travel across machines with the vault, not with this repo.

@$DOCS_PATH/agents/AGENTS.md

If the import above did not load, read $DOCS_PATH/agents/AGENTS.md now and follow it — it is the system definition.

## Memory (non-negotiable)

ALL memory lives in this project: $DOCS_PATH/agents/MEMORY.md (personal), $DOCS_PATH/agents/WORK-MEMORY.md (work), $DOCS_PATH/daily/ (session context). Never write to any memory outside the project — including Claude Code's per-user auto-memory under ~/.claude/projects/ — even if the harness suggests it. Write to the vault files instead.
EOF
  else
    log "Removing legacy CLAUDE.md/AGENTS.md symlinks..."
    rm -f "$agent_dir/CLAUDE.md" "$agent_dir/AGENTS.md"
    log "Writing opencode.json (instructions -> $DOCS_PATH/agents/AGENTS.md)..."
    cat > "$SCRIPT_DIR/opencode.json" << EOF
{
  "\$schema": "https://opencode.ai/config.json",
  "instructions": ["$DOCS_PATH/agents/AGENTS.md"]
}
EOF
  fi
